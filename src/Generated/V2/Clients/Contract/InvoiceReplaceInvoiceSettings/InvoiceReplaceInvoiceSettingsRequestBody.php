...
class InvoiceReplaceInvoiceSettingsRequestBody
{
    /**
    ...
    public function getIsBankrupt(): ?bool
    {
        return $this->isBankrupt ?? null;
    }
    ...
    public function withIsBankrupt(bool $isBankrupt): self
    {
        $validator = new Validator();
        $validator->validate($isBankrupt, static::$schema['properties']['isBankrupt']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }

        $clone = clone $this;
        $clone->isBankrupt = $isBankrupt;

        return $clone;
    }

    public function withoutIsBankrupt(): self
    {
        $clone = clone $this;
        unset($clone->isBankrupt);
     ...
     * @return InvoiceReplaceInvoiceSettingsRequestBody Created instance
     * @throws InvalidArgumentException
     */
    public static function buildFromInput(array|object $input, bool $validate = true): InvoiceReplaceInvoiceSettingsRequestBody
    {
        ...
        $isBankrupt = null;
        if (isset($input->{'isBankrupt'})) {
            $isBankrupt = (bool)($input->{'isBankrupt'});
        }
    ...
    public function toJson(): array
    {
        ...
        if (isset($this->isBankrupt)) {
            $output['isBankrupt'] = $this->isBankrupt;
        }
