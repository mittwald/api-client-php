...
class ListArticlesRequest
{
    ...
    {
        $validator = new Validator();
        $validator->validate($articleIds, self::$internalValidationSchema['properties']['articleIds']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }
    ...
    {
        $validator = new Validator();
        $validator->validate($orderable, self::$internalValidationSchema['properties']['orderable']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }
