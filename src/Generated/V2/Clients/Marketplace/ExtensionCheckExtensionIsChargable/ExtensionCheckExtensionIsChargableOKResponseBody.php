<?php

declare(strict_types=1);

namespace Mittwald\ApiClient\Generated\V2\Clients\Marketplace\ExtensionCheckExtensionIsChargable;

use InvalidArgumentException;
use JsonSchema\Validator;
use Mittwald\ApiClient\Generated\V2\Schemas\Marketplace\ExtensionInstanceChargeability;

class ExtensionCheckExtensionIsChargableOKResponseBody
{
    /**
     * Schema used to validate input for creating instances of this class
     */
    private static array $internalValidationSchema = [
        'properties' => [
            'chargeability' => [
                '$ref' => '#/components/schemas/de.mittwald.v1.marketplace.ExtensionInstanceChargeability',
            ],
        ],
        'required' => [
            'chargeability',
        ],
        'type' => 'object',
    ];

    private ExtensionInstanceChargeability $chargeability;

    public function __construct(ExtensionInstanceChargeability $chargeability)
    {
        $this->chargeability = $chargeability;
    }

    public function getChargeability(): ExtensionInstanceChargeability
    {
        return $this->chargeability;
    }

    public function withChargeability(ExtensionInstanceChargeability $chargeability): self
    {
        $clone = clone $this;
        $clone->chargeability = $chargeability;

        return $clone;
    }

    /**
     * Builds a new instance from an input array
     *
     * @param array|object $input Input data
     * @param bool $validate Set this to false to skip validation; use at own risk
     * @return ExtensionCheckExtensionIsChargableOKResponseBody Created instance
     * @throws InvalidArgumentException
     */
    public static function buildFromInput(array|object $input, bool $validate = true): ExtensionCheckExtensionIsChargableOKResponseBody
    {
        $input = is_array($input) ? Validator::arrayToObjectRecursive($input) : $input;
        if ($validate) {
            static::validateInput($input);
        }

        $chargeability = ExtensionInstanceChargeability::buildFromInput($input->{'chargeability'}, validate: $validate);

        $obj = new self($chargeability);

        return $obj;
    }

    /**
     * Converts this object back to a simple array that can be JSON-serialized
     *
     * @return array Converted array
     */
    public function toJson(): array
    {
        $output = [];
        $output['chargeability'] = $this->chargeability->toJson();

        return $output;
    }

    /**
     * Validates an input array
     *
     * @param array|object $input Input data
     * @param bool $return Return instead of throwing errors
     * @return bool Validation result
     * @throws InvalidArgumentException
     */
    public static function validateInput(array|object $input, bool $return = false): bool
    {
        $validator = new \Mittwald\ApiClient\Validator\Validator();
        $input = is_array($input) ? Validator::arrayToObjectRecursive($input) : $input;
        $validator->validate($input, self::$internalValidationSchema);

        if (!$validator->isValid() && !$return) {
            $errors = array_map(function (array $e): string {
                return $e["property"] . ": " . $e["message"];
            }, $validator->getErrors());
            throw new InvalidArgumentException(join(", ", $errors));
        }

        return $validator->isValid();
    }

    public function __clone()
    {
    }
}
