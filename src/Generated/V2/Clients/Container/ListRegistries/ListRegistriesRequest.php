...
class ListRegistriesRequest
{
    ...
    {
        $validator = new Validator();
        $validator->validate($hasCredentials, static::$schema['properties']['hasCredentials']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }

        $clone = clone $this;
        $clone->hasCredentials = $hasCredentials;

        return $clone;
    }

    public function withoutHasCredentials(): self
    {
        $clone = clone $this;
        unset($clone->hasCredentials);
     ...
     * @return ListRegistriesRequest Created instance
     * @throws InvalidArgumentException
     */
    public static function buildFromInput(array|object $input, bool $validate = true): ListRegistriesRequest
    {
    ...
    public function buildRequestOptions(): array
    {
        $mapped = $this->toJson();
        $query = [];
        if (isset($mapped['hasCredentials'])) {
            $query['hasCredentials'] = $mapped['hasCredentials'];
        }
