<?php

declare(strict_types=1);

namespace Mittwald\ApiClient\Generated\V2\Schemas\Backup;

use InvalidArgumentException;
use JsonSchema\Validator;

/**
 * Auto-generated class for de.mittwald.v1.backup.ProjectBackupDirectory.
 *
 * DO NOT EDIT; this class was generated by the mittwald/api-client-builder package
 * (https://github.com/mittwald/api-client-php-builder). Please make any changes
 * there.
 *
 * @generated
 * @see https://github.com/mittwald/api-client-php-builder
 */
class ProjectBackupDirectory
{
    /**
     * Schema used to validate input for creating instances of this class
     */
    private static array $internalValidationSchema = [
        'properties' => [
            'absolutePath' => [
                'type' => 'string',
            ],
            'isDirectory' => [
                'type' => 'boolean',
            ],
            'isExecutable' => [
                'type' => 'boolean',
            ],
            'isFile' => [
                'type' => 'boolean',
            ],
            'isSymlink' => [
                'type' => 'boolean',
            ],
            'items' => [
                'items' => [
                    '$ref' => '#/components/schemas/de.mittwald.v1.backup.ProjectBackupDirectory',
                ],
                'type' => 'array',
            ],
            'name' => [
                'type' => 'string',
            ],
            'size' => [
                'format' => 'int64',
                'type' => 'integer',
            ],
            'target' => [
                'type' => 'string',
            ],
        ],
        'required' => [
            'absolutePath',
            'name',
            'size',
        ],
        'type' => 'object',
    ];

    private string $absolutePath;

    private ?bool $isDirectory = null;

    private ?bool $isExecutable = null;

    private ?bool $isFile = null;

    private ?bool $isSymlink = null;

    /**
     * @var ProjectBackupDirectory[]|null
     */
    private ?array $items = null;

    private string $name;

    private int $size;

    private ?string $target = null;

    public function __construct(string $absolutePath, string $name, int $size)
    {
        $this->absolutePath = $absolutePath;
        $this->name = $name;
        $this->size = $size;
    }

    public function getAbsolutePath(): string
    {
        return $this->absolutePath;
    }

    public function getIsDirectory(): ?bool
    {
        return $this->isDirectory ?? null;
    }

    public function getIsExecutable(): ?bool
    {
        return $this->isExecutable ?? null;
    }

    public function getIsFile(): ?bool
    {
        return $this->isFile ?? null;
    }

    public function getIsSymlink(): ?bool
    {
        return $this->isSymlink ?? null;
    }

    /**
     * @return ProjectBackupDirectory[]|null
     */
    public function getItems(): ?array
    {
        return $this->items ?? null;
    }

    public function getName(): string
    {
        return $this->name;
    }

    public function getSize(): int
    {
        return $this->size;
    }

    public function getTarget(): ?string
    {
        return $this->target ?? null;
    }

    public function withAbsolutePath(string $absolutePath): self
    {
        $validator = new Validator();
        $validator->validate($absolutePath, self::$internalValidationSchema['properties']['absolutePath']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }

        $clone = clone $this;
        $clone->absolutePath = $absolutePath;

        return $clone;
    }

    public function withIsDirectory(bool $isDirectory): self
    {
        $validator = new Validator();
        $validator->validate($isDirectory, self::$internalValidationSchema['properties']['isDirectory']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }

        $clone = clone $this;
        $clone->isDirectory = $isDirectory;

        return $clone;
    }

    public function withoutIsDirectory(): self
    {
        $clone = clone $this;
        unset($clone->isDirectory);

        return $clone;
    }

    public function withIsExecutable(bool $isExecutable): self
    {
        $validator = new Validator();
        $validator->validate($isExecutable, self::$internalValidationSchema['properties']['isExecutable']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }

        $clone = clone $this;
        $clone->isExecutable = $isExecutable;

        return $clone;
    }

    public function withoutIsExecutable(): self
    {
        $clone = clone $this;
        unset($clone->isExecutable);

        return $clone;
    }

    public function withIsFile(bool $isFile): self
    {
        $validator = new Validator();
        $validator->validate($isFile, self::$internalValidationSchema['properties']['isFile']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }

        $clone = clone $this;
        $clone->isFile = $isFile;

        return $clone;
    }

    public function withoutIsFile(): self
    {
        $clone = clone $this;
        unset($clone->isFile);

        return $clone;
    }

    public function withIsSymlink(bool $isSymlink): self
    {
        $validator = new Validator();
        $validator->validate($isSymlink, self::$internalValidationSchema['properties']['isSymlink']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }

        $clone = clone $this;
        $clone->isSymlink = $isSymlink;

        return $clone;
    }

    public function withoutIsSymlink(): self
    {
        $clone = clone $this;
        unset($clone->isSymlink);

        return $clone;
    }

    /**
     * @param ProjectBackupDirectory[] $items
     */
    public function withItems(array $items): self
    {
        $clone = clone $this;
        $clone->items = $items;

        return $clone;
    }

    public function withoutItems(): self
    {
        $clone = clone $this;
        unset($clone->items);

        return $clone;
    }

    public function withName(string $name): self
    {
        $validator = new Validator();
        $validator->validate($name, self::$internalValidationSchema['properties']['name']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }

        $clone = clone $this;
        $clone->name = $name;

        return $clone;
    }

    public function withSize(int $size): self
    {
        $validator = new Validator();
        $validator->validate($size, self::$internalValidationSchema['properties']['size']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }

        $clone = clone $this;
        $clone->size = $size;

        return $clone;
    }

    public function withTarget(string $target): self
    {
        $validator = new Validator();
        $validator->validate($target, self::$internalValidationSchema['properties']['target']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }

        $clone = clone $this;
        $clone->target = $target;

        return $clone;
    }

    public function withoutTarget(): self
    {
        $clone = clone $this;
        unset($clone->target);

        return $clone;
    }

    /**
     * Builds a new instance from an input array
     *
     * @param array|object $input Input data
     * @param bool $validate Set this to false to skip validation; use at own risk
     * @return ProjectBackupDirectory Created instance
     * @throws InvalidArgumentException
     */
    public static function buildFromInput(array|object $input, bool $validate = true): ProjectBackupDirectory
    {
        $input = is_array($input) ? Validator::arrayToObjectRecursive($input) : $input;
        if ($validate) {
            static::validateInput($input);
        }

        $absolutePath = $input->{'absolutePath'};
        $isDirectory = null;
        if (isset($input->{'isDirectory'})) {
            $isDirectory = (bool)($input->{'isDirectory'});
        }
        $isExecutable = null;
        if (isset($input->{'isExecutable'})) {
            $isExecutable = (bool)($input->{'isExecutable'});
        }
        $isFile = null;
        if (isset($input->{'isFile'})) {
            $isFile = (bool)($input->{'isFile'});
        }
        $isSymlink = null;
        if (isset($input->{'isSymlink'})) {
            $isSymlink = (bool)($input->{'isSymlink'});
        }
        $items = null;
        if (isset($input->{'items'})) {
            $items = array_map(fn (array|object $i): ProjectBackupDirectory => ProjectBackupDirectory::buildFromInput($i, validate: $validate), $input->{'items'});
        }
        $name = $input->{'name'};
        $size = (int)($input->{'size'});
        $target = null;
        if (isset($input->{'target'})) {
            $target = $input->{'target'};
        }

        $obj = new self($absolutePath, $name, $size);
        $obj->isDirectory = $isDirectory;
        $obj->isExecutable = $isExecutable;
        $obj->isFile = $isFile;
        $obj->isSymlink = $isSymlink;
        $obj->items = $items;
        $obj->target = $target;
        return $obj;
    }

    /**
     * Converts this object back to a simple array that can be JSON-serialized
     *
     * @return array Converted array
     */
    public function toJson(): array
    {
        $output = [];
        $output['absolutePath'] = $this->absolutePath;
        if (isset($this->isDirectory)) {
            $output['isDirectory'] = $this->isDirectory;
        }
        if (isset($this->isExecutable)) {
            $output['isExecutable'] = $this->isExecutable;
        }
        if (isset($this->isFile)) {
            $output['isFile'] = $this->isFile;
        }
        if (isset($this->isSymlink)) {
            $output['isSymlink'] = $this->isSymlink;
        }
        if (isset($this->items)) {
            $output['items'] = array_map(fn (ProjectBackupDirectory $i): array => $i->toJson(), $this->items);
        }
        $output['name'] = $this->name;
        $output['size'] = $this->size;
        if (isset($this->target)) {
            $output['target'] = $this->target;
        }

        return $output;
    }

    /**
     * Validates an input array
     *
     * @param array|object $input Input data
     * @param bool $return Return instead of throwing errors
     * @return bool Validation result
     * @throws InvalidArgumentException
     */
    public static function validateInput(array|object $input, bool $return = false): bool
    {
        $validator = new \Mittwald\ApiClient\Validator\Validator();
        $input = is_array($input) ? Validator::arrayToObjectRecursive($input) : $input;
        $validator->validate($input, self::$internalValidationSchema);

        if (!$validator->isValid() && !$return) {
            $errors = array_map(function (array $e): string {
                return $e["property"] . ": " . $e["message"];
            }, $validator->getErrors());
            throw new InvalidArgumentException(join(", ", $errors));
        }

        return $validator->isValid();
    }

    public function __clone()
    {
    }
}
