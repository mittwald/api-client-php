...
class ArticleTemplate
{
    ...
    {
        $validator = new Validator();
        $validator->validate($isManagedByDomain, self::$internalValidationSchema['properties']['isManagedByDomain']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }
    ...
    {
        $validator = new Validator();
        $validator->validate($isRecurring, self::$internalValidationSchema['properties']['isRecurring']);
        if (!$validator->isValid()) {
            throw new InvalidArgumentException($validator->getErrors()[0]['message']);
        }
